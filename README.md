...
### 5. 作品管理
- 学员端提交作品（标题、描述、本地文件上传至 `app/static/uploads/`）
- 文件上传限制：jpg/jpeg/png/webp，最大 5 MB，安全文件名+随机前缀防重名
- 教师端点评作品（评分、评语）
- 作品截止提醒（24小时内到期预警）
- 作品删除（软删除）：学员可删未被点评的自己作品；教师可删本班作品；管理员可删任意作品；删除同步移除磁盘文件并写入审计日志

### 8. 学员自助注册
- 学员可通过 `/register` 页面自助注册账号
- 注册后状态为 `pending`（待审核），无法登录使用核心功能
- 管理员在"用户与权限管理"后台审核通过（改为 `approved`）后方可正常使用
